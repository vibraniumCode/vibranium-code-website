<template>
  <div class="relative isolate bg-[#1d232a] mt-12 px-6 py-24 sm:py-32 lg:px-8">
    ...
    <div class="mx-auto max-w-4xl text-center">
      ...
      <p class="mt-2 text-balance text-5xl font-semibold tracking-tight text-white sm:text-6xl">Elige el plan adecuado para ti</p>
    </div>
    <p class="mx-auto mt-6 max-w-2xl text-pretty text-center text-lg font-medium text-gray-600 sm:text-xl/8">Elige el plan perfecto para hacer crecer tu negocio</p>
    <div class="mx-auto mt-16 grid max-w-lg grid-cols-1 items-center gap-y-6 sm:mt-20 sm:gap-y-0 lg:max-w-6xl lg:grid-cols-3">
      <div v-for="(tier, tierIdx) in tiers" :key="tier.id" :class="[tier.featured ? 'relative bg-gray-900 shadow-2xl' : 'bg-white sm:mx-8 lg:mx-0', tier.featured ? '' : tierIdx === 0 ? 'rounded-t-3xl sm:rounded-b-none lg:rounded-bl-3xl lg:rounded-tr-none' : 'sm:rounded-t-none lg:rounded-bl-none lg:rounded-tr-3xl', 'rounded-3xl p-8 ring-1 ring-gray-900/10 sm:p-10']">
...
const tiers = [
  ...
  },
  {
    ...
    id: 'tier-enterprise',
    href: '#',
    priceMonthly: 'Desde ARS $180.000',
